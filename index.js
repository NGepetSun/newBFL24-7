...
async function checkViaRSS(channelId) {
  ...
  // Ambil nama channel dari tag <name> di RSS (lebih akurat dari <title>)
  const nameTagMatch = xml.match(/<name>([^<]+)<\/name>/);
  ...
  // Cek 3 video terbaru dari RSS (live stream kadang tidak selalu di posisi 0)
...
async function checkViaLivePage(channelId) {
  // Coba dua URL format: channel ID dan handle
  let html = null;
  const urls = [
    `https://www.youtube.com/channel/${channelId}/live`,
  ];
  // Jika channelId bukan format UC... maka treat sebagai handle
  ...
  // Hanya gunakan sinyal EKSPLISIT offline — jangan terlalu agresif
...
async function getChannelName(channelId) {
  try {
    ...
    // RSS YouTube punya tag <name> yang isinya PERSIS nama channel
    // Lebih reliable dari <title> yang bisa berisi judul video
    const nameTag = xml.match(/<name>([^<]+)<\/name>/);
    if (nameTag && nameTag[1].trim()) return nameTag[1].trim();

    // Fallback: <title> di dalam <author> block
    const authorBlock = xml.match(/<author>[\s\S]*?<name>([^<]+)<\/name>[\s\S]*?<\/author>/);
    if (authorBlock && authorBlock[1].trim()) return authorBlock[1].trim();

    // Fallback: scrape halaman channel langsung
...
// Fallback: scrape nama channel dari halaman YouTube
async function getChannelNameFromPage(channelId) {
  try {
    ...
    // "channelMetadataRenderer" berisi nama channel yang akurat
    ...
    // Fallback: <title> tag (biasanya "Nama Channel - YouTube")
...
async function checkChannelLive(channelId) {
  ...
  try {
    ...
    // Simpan nama channel dari RSS meskipun tidak live
  ...
  // Jika nama belum dapat dari RSS, coba ambil dari halaman channel
...
async function checkAll() {
  ...
  for (const id of CHANNEL_IDS) {
    // Init state
    if (!state[id]) {
      state[id] = { name: id, isLive: false, videoId: null, lastNotifAt: null };
      // Ambil nama channel secara async — tidak block monitor
      getChannelName(id).then(name => {
        if (state[id]) {
          state[id].name = name;
          log('INFO', `📺 Nama channel: ${name} (${id})`);
        }
      });
    }
    const ch = state[id];
    ...
    try {
      ...
      // Selalu update nama dari hasil deteksi — paling akurat
